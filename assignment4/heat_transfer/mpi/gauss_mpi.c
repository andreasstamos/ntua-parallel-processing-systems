...
int main(int argc, char ** argv) {
    ...
    */
    ...
    for (t=0;t<T;t++) {
        ...
        MPI_Irecv(&u_current[0][1],          1, box_boundary_row, north, 1, CART_COMM, &r[0]);
        MPI_Irecv(&u_current[1][0],          1, box_boundary_col, west,  3, CART_COMM, &r[1]);
        ...
        MPI_Isend(&u_current[local[0]][1],   1, box_boundary_row, south, 1, CART_COMM, &r[0]);
        MPI_Isend(&u_current[1][local[1]],   1, box_boundary_col, east,  3, CART_COMM, &r[1]);

        MPI_Isend(&u_current[1][1],          1, box_boundary_row, north, 0, CART_COMM, &r[2]);
        MPI_Isend(&u_current[1][1],          1, box_boundary_col, west,  2, CART_COMM, &r[3]);
       
        MPI_Irecv(&u_current[local[0]+1][1], 1, box_boundary_row, south, 0, CART_COMM, &r[4]);
        MPI_Irecv(&u_current[1][local[1]+1], 1, box_boundary_col, east,  2, CART_COMM, &r[5]);

        MPI_Waitall(6, r, MPI_STATUSES_IGNORE);
