...
int main(int argc, char **argv)
{
    ...
    // Broadcast initial cluster positions to all ranks
    ...
    // Fixed number of objects that this kmeans function call will process
    ...
    */
    ...
    // Calculate recvcounts and displs, which will be used to gather data from each rank.
    // Hint: recvcounts: number of elements received from each rank
    //       displs: displacement of each rank's data
    if (rank == 0) {
        int perRank = numObjs / size;
        displs[0] = 0;
        recvcounts[0] = (numObjs % size >= 1 ? perRank + 1 : perRank);
        for (int i=1; i<numObjs % size; ++i) {
            displs[i] = displs[i-1] + recvcounts[i-1];
            recvcounts[i] = perRank + 1;
        }
        for (int i=MAX(1, numObjs%size); i<size; ++i) {
            recvcounts[i] = perRank;
            displs[i] = displs[i-1] + recvcounts[i-1];
        }
    }
 
    // Broadcasting of recvcounts and displs arrays to other ranks is not necessary.

    // Gather membership information from every rank. (hint: each rank may send different number of objects)
