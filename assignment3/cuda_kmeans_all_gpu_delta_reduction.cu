...
void find_nearest_cluster(int numCoords,
  ...
  // we do warp-level reduction using warp-level primitives.
  unsigned deltas = __ballot_sync(0xffffffff, (tid<numObjs && deviceMembership[tid] != index));
  if (threadIdx.x % 32 == 0) {
    shmemDelta[threadIdx.x / 32] = __popc(deltas); // max thread block size = 32 warps (aka 1024 threads), therefore an array of 32 elements is enough.
  }
  ...
  // shmemDeltas has size 32 (=max threadblock size = 32 warps), therefore it can be reduced by just 1 warp, using a warp reduction.
  // we do a tree reduction inside just 1 warp, using warp-level primitives.
  if (threadIdx.x / 32 == 0) {
    
    // only warp 0
    int res = (threadIdx.x < blockDim.x / 32) ? shmemDelta[threadIdx.x % 32] : 0; // getting the values.
    for (int i = 16; i > 0; i /= 2)
      res += __shfl_down_sync(0xffffffff, res, i); //"sending" to the "lower half" each time.

    // the 0th thread of the 0th warp now has the result (for the whole thread block).
    if (threadIdx.x==0) atomicAdd(devdelta, res);
  }
