...
template <typename T>
__device__ void reduceArrayFromBlocks(T *arr, int size, __shared__ T *shmemReduction) {
  // we store at arr[block=0] the results.
  // shmemReduction has to be of size 32.

  for (int j=0; j<size; ++j) {
    T sum = 0;
    //we do a reduction so that each thread of the last threadblock gets a number
    for (int block=threadIdx.x; block<gridDim.x; block += blockDim.x)
      sum += arr[block*size + j];

    // now we do threadblock reduction as above.

    // firstly a warp reduction
    for (int i = 16; i > 0; i /= 2)
      sum += __shfl_down_sync(0xffffffff, sum, i); //"sending" to the lowest thread

    // the 1st thread of each warp now has the result. we store to a block of shared memory.
    if (threadIdx.x % 32 == 0)
      shmemReduction[threadIdx.x / 32] = sum; // max block size = 1024, warp size = 32, therefore an array of 1024/32=32 elements is enough.

    __syncthreads();

    // another warp reduction by the 1st warp.
    if (threadIdx.x / 32 == 0) {
      sum = (threadIdx.x < blockDim.x / 32) ? shmemReduction[threadIdx.x % 32] : 0; // getting the values.

      for (int i = 16; i > 0; i /= 2)
        sum += __shfl_down_sync(0xffffffff, sum, i); //"sending" to the lowest thread

      // the 1st thread of the 1st warp now has the result.
      if (threadIdx.x==0) arr[j] = sum;
    }
  }
}

__device__ static unsigned int finished = 0;
...
void find_nearest_cluster(int numCoords,
                          ...
                          double *devicenewClusters, // [gridDim][numCoords*numClusters]
                          int *devicenewClusterSize,           //  [gridDim][numClusters]
                          int *deviceMembership,
                          double *devdelta) { // [gridDim][1].
  ...
  int *shmemClusterSizes = reinterpret_cast<int*>(&shmemDelta[32]);
  ...
  unsigned deltas = __ballot_sync(0xffffffff, (tid<numObjs && deviceMembership[tid] != index));
  if (threadIdx.x % 32 == 0) {
    shmemDelta[threadIdx.x / 32] = __popc(deltas); // max block size = 32, therefore an array of 32 elements is enough.
    // max block size = 32, therefore we can do a warp reduction (by just one warp).
  }
  __syncthreads(); // we have to exit the if, to ensure syncthreads is called for all threads. (otherwise we would get undefined behaviour.)
  if (threadIdx.x / 32 == 0) {
    // only warp 1
    int res = (threadIdx.x < blockDim.x / 32) ? shmemDelta[threadIdx.x % 32] : 0; // getting the values.
    for (int i = 16; i > 0; i /= 2)
      res += __shfl_down_sync(0xffffffff, res, i); //"sending" to the lowest thread

    // the 1st thread of the 1st warp now has the result.
    if (threadIdx.x==0) devdelta[blockIdx.x] = res;
  }

  if (tid < numObjs) deviceMembership[tid] = index;
  ...
  for (int i=threadIdx.x; i<numCoords*numClusters; i+=blockDim.x) devicenewClusters[blockIdx.x* numCoords*numClusters + i] = shmemClusters[i];
  for (int i=threadIdx.x; i<numClusters; i+=blockDim.x) devicenewClusterSize[blockIdx.x* numClusters +i] = shmemClusterSizes[i];

  __threadfence(); //TODO: why?

  __shared__ bool amLast;
  if (threadIdx.x == 0) {
    unsigned int ticket = atomicInc(&finished, gridDim.x);
  ...
  if (amLast) {
    finished = 0;
    reduceArrayFromBlocks(devicenewClusters, numClusters*numCoords, (double*) shmemBlock);
    reduceArrayFromBlocks(devicenewClusterSize, numClusters, (int*) shmemBlock);
    reduceArrayFromBlocks(devdelta, 1, (double*) shmemBlock);
  }
}
...
void kmeans_gpu(double *objects,      /* in: [numObjs][numCoords] */
  ...
  const unsigned int clusterBlockSharedDataSize = numCoords*numClusters*sizeof(double) + numClusters*sizeof(int) + 32*sizeof(double);
  ...
  checkCuda(cudaMalloc(&devicenewClusters, numClusterBlocks * numClusters * numCoords * sizeof(double)));
  ...
  checkCuda(cudaMemset(devicenewClusterSize, 0, numClusterBlocks * numClusters * sizeof(int)));
  checkCuda(cudaMemset(devicenewClusters, 0, numClusterBlocks * numClusters * numCoords * sizeof(double)));
  free(dimObjects[0]);
  ...
  fprintf(fp, "%s,%d,%lf,%lf,%lf\n", "All_GPU_GlobalReduction", blockSize, timing / loop, timer_min, timer_max);
  fclose(fp);
